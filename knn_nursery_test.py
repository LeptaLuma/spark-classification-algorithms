...
def test_nursery_knn(k=5):
    ...
    rdd_metrics = benchmark_function(
        "RDD Implementation",
        knn_rdd, sc, train_rdd, test_rdd, k, n_partitions
    )
    
    # Calculate accuracy
    rdd_preds = [pred for _, pred in sorted(rdd_metrics['result'], key=lambda x: x[0])]
    ...
    df_metrics = benchmark_function(
        ...
        knn_df, spark, train_data, test_data, k, n_partitions
    )
    
    # Calculate accuracy
    df_preds = [row.predicted_class for row in sorted(df_metrics['result'], key=lambda r: r.id_test)]
